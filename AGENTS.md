...
本节补充人机协作层面的流程要求，与前述架构规范并行有效：架构规范约束“改什么”，本节约束“怎么改、怎么交付”。仓库通过 Cline
在 VS Code 中协作，开发者本机无 Android Studio，只能真机验证。

1. 每次改动后必须创建对应的 Git Commit，便于追踪和回滚。禁止在没有 Commit 的情况下继续下一步任务。
2. 严禁重写整个文件；修改必须基于现有代码结构，尽量缩小改动范围（只动必要的行）。
3. 严禁擅自修改 `build.gradle.kts` 里的依赖版本、AGP 版本或项目配置，除非用户明确要求。
4. 每次收到任务，必须先阅读相关文件，输出《修改方案》（明确列出将修改哪些文件、改哪几行、为什么这么改），并等待用户回复“确认”后才能动手写代码。
5. 修改完成后，必须同步更新相关测试代码（如果项目中存在对应测试）。
6. 交付前必须完成自检：
   - 语法自检：确保没有语法错误、导包缺失、括号不匹配、方法签名不匹配。
   - 逻辑自检：确认代码真正实现了需求，未引入空指针、越界等风险。
   - 静态验证：尝试运行 `./gradlew.bat :app:compileAppDebugKotlin` 或本文档「构建与验证」一节列出的命令；报错必须修复；因环境限制无法运行时，必须在交付说明中明确告知“无法本地编译，需真机验证”。
7. 交付时必须按以下格式回复：
   - [已修改文件列表]
   - [核心改动逻辑简述]
   - [自检结果（通过/未运行）]
